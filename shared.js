...
const Auth = {
  ...
  register(name, email, password) {
    ...
    const newUser = {
      id:"u"+Date.now(), name, email, password:btoa(password),
...
const DEFAULT_COURSES = [
  {
    ...
    thumbnail:"",instructor:"Tim Kaizo",duration:"3 jam",
    ...
    modules:[
      ...
      {id:"m2",title:"Permintaan & Penawaran",lessons:[
        ...
        {id:"l5",title:"Hukum Penawaran",type:"text",duration:"12 min",content:"**Hukum Penawaran:** Jika harga suatu barang naik, maka jumlah yang ditawarkan akan naik (ceteris paribus).\n\nFaktor: harga barang, biaya produksi, teknologi, harga input, ekspektasi produsen"}
      ]}
    ]
  },
  {
    id:"c2",title:"Matematika Dasar Kelas 9",category:"Matematika",level:"Beginner",
    thumbnail:"",instructor:"Tim Kaizo",duration:"4 jam",
    description:"Kuasai konsep matematika kelas 9: aljabar, geometri, dan statistika dasar.",
    rating:4.6,students:189,xpReward:250,
    modules:[
      {id:"m3",title:"Aljabar",lessons:[
        {id:"l6",title:"Persamaan Linear",type:"text",duration:"15 min",content:"**Persamaan Linear Satu Variabel** adalah persamaan yang memuat satu variabel dengan pangkat tertinggi 1.\n\nBentuk umum: **ax + b = c**\n\nContoh: 2x + 3 = 7 → x = 2"},
        {id:"l7",title:"Sistem Persamaan",type:"text",duration:"15 min",content:"**SPLDV** diselesaikan dengan:\n1. Substitusi\n2. Eliminasi\n3. Grafik"}
      ]},
      {id:"m4",title:"Statistika",lessons:[
        {id:"l8",title:"Mean, Median, Modus",type:"text",duration:"12 min",content:"**Mean:** jumlah data dibagi banyak data\n**Median:** nilai tengah data yang diurutkan\n**Modus:** nilai yang paling sering muncul"}
      ]}
    ]
  }
];
...
function getCourseProgress(courseId) {
  const user = Auth.getCurrent(); if(!user) return 0;
  const users = Auth.getUsers(); const u = users.find(x=>x.id===user.id); if(!u) return 0;
  const course = getCourses().find(c=>c.id===courseId); if(!course) return 0;
  const all = course.modules.flatMap(m=>m.lessons);
  const done = all.filter(l=>(u.completedLessons||[]).includes(`${courseId}:${l.id}`)).length;
...
const MEDAL = ["🥇","🥈","🥉"];
const CAT_ICONS = {Ekonomi:"📊",Matematika:"🧮",Bahasa:"✍️",Sains:"🔬",default:"📚"};
